...
class Query
{
	...
	public function build()
	{
		...
		return array($this->_sSQL,$this->_aBindParams);
	}
	...
	protected function _buildGroupBy()
	{
		...
		{
			$tmp = array();
			foreach($this->_aGroupBys as $key => $aGroup)
			{
	...
	protected function _buildJoin()
	{
		...
		if(count($this->_aJoins))
		{
			...
			{
				$join .= " JOIN ". $aJoin['type'] ." ". $aJoin['table'];
	...
	protected function _buildWhere()
	{
		...
		if(count($this->_aConds))
		{
			foreach($this->_aConds as $key => $aCond)
			{
				$where = $aCond['cond']. " " ; 
				$bHasSubQuery = false;
				if($aCond['bind'] instanceof  \API\Engine\Database\Query)
				{
	...
	protected function _buildFrom()
	{
		...
		if(!empty($this->_aMainFrom['alias']))
		{
	...
	{
		$iPage = isset($this->_aPager['page'])?$this->_aPager['page']:null;
		$iLimit = isset($this->_aPager['limit'])?$this->_aPager['limit']:null;
